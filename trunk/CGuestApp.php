...
class GuestApp {
    ...
    public $APIPath;
    private $token;
    private $allowedLocales = array('it', 'fr', 'en', 'es', 'nl', 'de', 'ru', 'tr', 'jp');
    ...
    public function getReviewData($json) {
        ...
        foreach ($json->reviews as $review) {
            ...
            $data = array(
                ...
                "ratings_by_language" => isset($review->ratings_by_language) ? $review->ratings_by_language : new stdClass(),
    ...
    public function getRatingNameAndValue($ratingKey, $ratingData)
    {
        $language = $this->getLanguage();

        if( isset($ratingData->value) && is_numeric($ratingData->value) ) {
            // Case 1 : New API version (with translations)
            $ratingName = isset($ratingData->$language) && $ratingData->$language ? $ratingData->$language : $ratingKey;
            $ratingValue = $ratingData->value;
        }

        return array($ratingName, $ratingValue);
    }

    public function getCleanReview($review) {

        if (isset($review['ratings_by_language'])) {
            // Case 1: new API version
            $review['ratings'] = array();
            foreach ($review['ratings_by_language'] as $ratingKey => $ratingData) {
                list($ratingName, $ratingValue) = $this->getRatingNameAndValue($ratingKey, $ratingData);
                $review['ratings'][$ratingName] = $ratingValue;
            }
        }
    ...
    public function getAverageData($json) {
        ...
        for ($i = 0; $i < $json->count; $i++) {
            ...
            $ratignsToExtract = array();
            if (isset($json->reviews[$i]->ratings_by_language)) {
                // Case 1: new API version
                $ratignsToExtract = $json->reviews[$i]->ratings_by_language;
            } else if (isset($json->reviews[$i]->ratings)) {
                // Case 2: old API version
                $ratignsToExtract = $json->reviews[$i]->ratings;
            }

            foreach($ratignsToExtract as $key => $ratingData) {

                list($ratingName, $ratingValue) = $this->getRatingNameAndValue($key, $ratingData);

                // If this subrating already exists in our list, we simply add to it
                // otherwise, we create it.
                if (isset($sub_ratings[$ratingName])) {
                    $sub_ratings[$ratingName]->total += $ratingValue;
                    $sub_ratings[$ratingName]->count += 1;
                } else {
                    $newRating = new stdClass();
                    $newRating->total = $ratingValue;
                    $newRating->count = 1;
                    $sub_ratings[$ratingName] = $newRating;
                }
            }
